    ...
    actions: {
        ...
        cancelPremium: function () {
            var confirm = {
                positive: "Yes",
                negative: "No",
                callbackAction: "cancelPremiumConfirmation",
                controller: this
            };
            var expiryDate = new moment(this.get('currentUser.privateData.premiumNextPayment'));
            this.send('addNotification', 'warning', 'Are you sure?',
                'Your subscription will end on ' +  expiryDate.format("Do MMM YYYY") + ".", confirm);

        },
        cancelPremiumConfirmation: function (isPositive) {
            if (!isPositive)
                return;
            Parse.Cloud.run('cancelSubscription', {})
                .then(function (response) {
                    var expiryDate = new moment(this.get('currentUser.privateData.premiumExpiryDate'));
                    this.get('currentUser.privateData.content').reload();
                    this.send('addNotification', 'srs', 'Premium Cancelled!', 'Your subscription will end on '
                    + expiryDate.format("Do MMM YYYY"));
